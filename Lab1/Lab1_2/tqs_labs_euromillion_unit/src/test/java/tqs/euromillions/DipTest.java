...
public class DipTest {
    ...
    @DisplayName("new Dip rejects invalid number of elements")
    @Test
    public void testConstructorFromBadSize() {
        // creating Dip with numbers or starts outside the expected range
        // expects an exception
        assertThrows(IllegalArgumentException.class,
                () -> new Dip( new int[]{10, 11, 12, 13, 14, 15}, new int[]{1,2} ) );
        assertThrows(IllegalArgumentException.class,
                () -> new Dip( new int[]{11, 12, 13, 14, 15}, new int[]{ 1, 2, 3} ) );

        assertThrows(IllegalArgumentException.class,
                () -> new Dip( new int[]{11, 12, 13, 14}, new int[]{ 1, 2} ) );
        assertThrows(IllegalArgumentException.class,
                () -> new Dip( new int[]{11, 12, 13, 14, 15}, new int[]{ 1} ) );
    ...
    public void testGenerateRandomDip() {
        Dip d = Dip.generateRandomDip();
        assertEquals(Dip.NUMBERS_REQUIRED, d.getNumbersColl().size());
        assertEquals(Dip.STARS_REQUIRED, d.getStarsColl().size());
    }
