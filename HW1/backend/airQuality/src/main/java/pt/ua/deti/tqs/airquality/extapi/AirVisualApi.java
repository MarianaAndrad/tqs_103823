...
public class AirVisualApi {
    ...
    public List<String> getCountries() throws URISyntaxException, IOException {
        ...
        log.debug("\tGet Countries - Response: {}", response);

        JSONObject responseJson = new JSONObject(response);

        if (!responseJson.getString("status").equals("success")) {
            JSONObject data = responseJson.getJSONObject("data");
            log.error("\t[Error] getting countries from AirVisual API: {}", data.get("message"));
            return null;
        }
    ...
    public List<String> getStates(String country) throws URISyntaxException, IOException {
        ...
        log.debug("List State - Response: {}", response);

        JSONObject responseJson = new JSONObject(response);

        if (responseJson.getString("status").equals("fail")) {
            JSONObject data = responseJson.getJSONObject("data");
            log.error("\t[Error] getting states from AirVisual API: {}", data.get("message"));
            return null;
        }
    ...
    public List<String> getCities(String country, String state) throws URISyntaxException, IOException {
        ...
        log.info("\t[REQUEST] Getting List the Cities By State: {} and Country: {}", state, country);
        URIBuilder builder = new URIBuilder(url.replace(" ", "%20"));
        String response = constructUrlRequest(builder.build().toString(),1);
        log.debug("\t List Cities - Response: {}", response);

        JSONObject responseJson = new JSONObject(response);

        if (responseJson.getString("status").equals("fail")) {
            JSONObject data = responseJson.getJSONObject("data");
            log.error("\t[Error] getting cities from AirVisual API: {}", data.get("message"));
            return null;
        }
    ...
    public WeatherEntry getWeather(String city, String state, String country) throws URISyntaxException, IOException {
        ...
        if (responseJson.getString("status").equals("fail")) {
            ...
            log.info("\t[Error] getting weather from AirVisual API: {}", data.get("message"));
            return null;
        }
